...
Geometry sample SVGs live under `docs/gallery/` (20 variants per small sample, seeds 42–61; large convex polygons use a single fixed preview; **committed** so CI skips regeneration). Regenerate locally with `make visualize`, then commit `docs/gallery/*.svg`. Each small sample has a ↻ button — click to cycle, Shift-click for a random variant.
